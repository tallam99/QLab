...
CREATE TABLE slots (
    ...
    FOREIGN KEY (lab_id, user_id)
        REFERENCES lab_memberships (lab_id, user_id) ON DELETE CASCADE,
    -- When assigned, the resource is one of THIS slot's pool (not another pool's).
    -- Composite-FK MATCH SIMPLE: with assigned_resource_id NULL the rule is
    -- inactive, so unassigned slots are unconstrained.
    FOREIGN KEY (assigned_resource_id, resource_pool_id)
        REFERENCES resources (id, resource_pool_id),

    -- An ACTIVE slot is pinned: it must have a concrete resource and start time.
    CONSTRAINT slots_active_pinned CHECK (
        status <> 'ACTIVE'
        OR (assigned_resource_id IS NOT NULL AND actual_start IS NOT NULL)
    )
);
...
-- enforces that and doubles as the queue-order lookup (resource_pool_id, priority).
CREATE UNIQUE INDEX slots_live_priority_uniq
    ON slots (resource_pool_id, slot_priority)
    WHERE status IN ('SCHEDULED', 'ACTIVE');

-- Per-resource timeline lookups (PLAN Phase 5).
CREATE INDEX slots_resource_timeline
    ON slots (assigned_resource_id, actual_start)
    WHERE assigned_resource_id IS NOT NULL;

-- Tenant scoping.
CREATE INDEX slots_lab ON slots (lab_id);
...
-- assigned_resource_id. DEFERRABLE INITIALLY DEFERRED so Phase 7's reschedule can
-- rewrite many rows in one transaction through transient overlaps and be checked
-- only at COMMIT. Settled/unassigned rows are excluded via the WHERE predicate.
-- At QLab's scale (a handful of resources, a short per-pool queue) the GiST check
-- is negligible; revisit only if a single pool's queue ever grows large.
