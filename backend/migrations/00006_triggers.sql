...
--     (updated_by is application-set: the DB can't know the acting user.)
...
CREATE TRIGGER labs_users_set_updated_at
    BEFORE UPDATE ON labs_users
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
BEGIN
    IF OLD.status = 'ACTIVE' THEN
        IF NEW.resources_id IS DISTINCT FROM OLD.resources_id THEN
            RAISE EXCEPTION 'cannot reassign an ACTIVE slot (slots_id=%)', OLD.slots_id
                USING ERRCODE = 'check_violation';
        END IF;
        IF NEW.actual_start IS DISTINCT FROM OLD.actual_start THEN
            RAISE EXCEPTION 'cannot move an ACTIVE slot''s start (slots_id=%)', OLD.slots_id
                USING ERRCODE = 'check_violation';
        END IF;
        ...
        IF NEW.status NOT IN ('ACTIVE', 'COMPLETE', 'CANCELLED') THEN
            RAISE EXCEPTION 'invalid status transition ACTIVE -> % (slots_id=%)',
                NEW.status, OLD.slots_id
                USING ERRCODE = 'check_violation';
        END IF;
    END IF;
    RETURN NEW;
...
DROP TRIGGER IF EXISTS labs_users_set_updated_at ON labs_users;
