...
- `internal/store/` — the data store: the business `Store` interface
  ...
  **engine-agnostic** — it never imports `dynamicqueue`; the scheduling layer
  ...
  `io.Closer`. squirrel will arrive only when a query is genuinely dynamic. A second
  interface, `store.OperatorStore` (same `*pgstore.Store`, but used over an elevated
  RLS-bypassing pool), holds the cross-tenant admin ops for the operator tooling
  (`CreateLabWorkspace`/`ListLabs`/`GetLabState`/`DeleteLab`/`UserByID`) — kept
  separate from the per-request `Store` on purpose (decision 0008). Mocks are
  generated by **mockery** into `store/mocks`, **not committed** — `mage genMocks` /
  `mage clearMocks`.
...
- `internal/services/operator/` — the staging/local-only operator domain service
  (interface + `v1`): provision demo workspaces, mint tokens to act as seeded users,
  list/inspect/tear down. Over `store.OperatorStore` (elevated, cross-tenant
  persistence) + an `operator.Minter` (decision 0008).
- `internal/devapi/` — the `qlab.dev.v1.DevService` Connect handler (transport for
  the operator service), gated by a constant-time `X-QLab-Operator-Secret`
  interceptor. A **separate** service from `qlab.v1`; mounted only outside production
  so the prod binary has no operator capability.
- `internal/clients/firebase/` — Firebase Auth client setup (the token verifier
  source; emulator auto-detected from `FIREBASE_AUTH_EMULATOR_HOST`) plus `Minter`,
  the email→ID-token primitive (custom token → Identity Toolkit exchange) behind the
  operator MintToken.
...
See `docs/runbook.md` → "Operator surface" for provisioning a workspace, getting a
token, and making an authenticated call. To run the binary directly, point it at Postgres and the Auth
emulator:
