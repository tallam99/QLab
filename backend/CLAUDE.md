...
- `internal/store/` — the data store: the business `Store` interface
  (`interface.go`), implemented by `store/pgstore`. Speaks the persistence domain
  (`Slot`/`Pool`/`Resource`/`OutboxRow`, full-lifecycle `SlotStatus`) and is
  **engine-agnostic** — it never imports `dynamicqueue`; the scheduling layer
  converts. Reads (`IsMember`/`PoolByID`/`SlotByID`/`ListSlots`) run inside a
  lab-scoped tx so RLS permits them; `WithPool` is the per-event unit of work (one
  tx: `SET LOCAL app.current_lab_id`, lock the pool's live slots `FOR UPDATE`, run
  the callback, upsert slots + enqueue outbox). `pgstore.New` pings on construction,
  so a returned store is guaranteed ready and nothing re-checks it; `pgstore.Store`
  is an `io.Closer` so the server can drain its pool on shutdown. Queries are
  hand-written pgx (squirrel will arrive when a query is genuinely dynamic). Mocks
  are generated by **mockery** (`.mockery.yaml`) into `store/mocks` but **not
  committed** — run `mage genMocks` (then `go mod tidy`) before building code that
  imports one; `mage clearMocks` removes them. The `//go:generate` directive lives
  on the interface.
- `internal/scheduling/` — the domain service between the API and the engine +
  store (both reached via interfaces; only data DTOs cross boundaries). One method
  per ALGORITHM §6 event; each mutating one runs inside `store.WithPool`, converts
  the locked live slots to `dynamicqueue` types, runs the single reschedule, writes
  placements back, and enqueues outbox rows for re-committed slots. Owns
  authorization (lab membership, owner-only, next-in-line for reclaims) and the
  injectable `Clock`. Errors are domain sentinels the API maps to Connect codes.
- `internal/principal/` — the caller identity (`UserID` + `LabID`) carried in
  request context; the seam auth populates. Phase 7 fills it from a dev header
  (`httpmw.DevPrincipal`, `X-QLab-User`/`X-QLab-Lab`); Phase 8 swaps in JWT
  verification populating the same shape. Lab is carried (not derived) because RLS is
...
- `integration_test/` — `integration`-tagged full-stack suite: it boots the real
  server (connecting as a non-privileged RLS-bound app role) against a throwaway DB
  and drives it through the generated Connect client + the dev-principal headers.
  The harness arranges state via a separate superuser pool, resets between tests
  (`TRUNCATE … RESTART IDENTITY CASCADE`), and injects a settable clock for
  deterministic time behaviour. Run via `mage testIntegration`; part of `mage test`.
  **This is where new end-to-end behaviour goes** — add cases (and multi-step
  chains) here as features land.
...
- `internal/api/` — the Connect service implementation (`qlab.v1.QlabService`): a
  thin transport adapter. Each method reads the caller principal, converts proto →
  domain, calls `internal/scheduling` (held behind an `atomic.Pointer`, set by
  `server.WithScheduling` once the store is ready — until then RPCs return
  `Unavailable`), and converts the result back. proto types live only here; domain
  errors map to Connect codes in `connectError`. `Service.Handler()` returns the
  mount path + handler; `server.New` mounts it.
- `internal/server/` — the server: router, handlers (methods on `Server`), and the
  ...
  (`WithPostgres` then `WithScheduling` — the latter builds the engine + scheduling
  service over the ready store and attaches it to the API), calls `Ready()` to flip
